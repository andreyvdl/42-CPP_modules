...
void  PmergeMe::fordJohnVec(std::vector<int>& arr)
{
  ...
  sortOnlyBiggers<std::vector<std::pair<int, int> >, std::vector<int> >(
    _kVec.begin(), _kVec.end(), _sVec, _extra
  );
...
void  PmergeMe::fordJohnDeq(std::deque<int>& arr)
{
  ...
  sortOnlyBiggers<std::deque<std::pair<int, int> >, std::deque<int> >(
    _kDeq.begin(), _kDeq.end(), _sDeq, _extra
  );
